...
class OrgHomeView(View):
    ...
    def get(self, request, org_id):
        ...
        teacher_course = Course.objects.all()[:1]
...
class TeacherDetailView(View):
    def get(self, request, teacher_id):
        ...
        if UserFavorite.objects.filter(user=request.user, fav_typ=3, fav_id=teacher.id):
            has_teacher_faved = True

        has_org_faved = False
        if UserFavorite.objects.filter(user=request.user, fav_typ=2, fav_id=teacher.org.id):
            has_org_faved = True
